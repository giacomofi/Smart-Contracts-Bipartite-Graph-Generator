...
class MyListener(SolidityListener):
    ...
    def enterContractDefinition(self, ctx: SolidityParser.ContractDefinitionContext):  # Override
        ...
        if str(ctx.children[0]) == 'abstract' or str(ctx.children[0]) == 'contract':
            if ctx.identifier() is None: # In case of empty definition
                pass
            else:
    ...
    def exploringPossibleCall(self, to_check):
        ...
        if len(possible_call) > 1:
            ...
            for p in possible_call:
                ...
                if flag is True:
                    if self.function_name is None: # In case of empty definition
                        pass
                    else:
    ...
    def write_csv(self, file_name, dapp_name, path):
        with open(f"{path}/{dapp_name}.csv", "a") as _file:
...
if __name__ == '__main__':
    ...
    # The path to the target directory containing all the decentralized applications.
    print("Insert your DApps dataset path here:")
    directory_path = input()
    i = 1
    # Use os.listdir() to get a list of all files and directories in the specified path
    contents = os.listdir(directory_path)
    # Filter the list to include only directories
    directories = [item for item in contents if os.path.isdir(os.path.join(directory_path, item))]
    print("In which path do you want to save CSVs?")
    result_path = input()
    for directory in directories:
        print(f"Analyzing directory: {directory}:{i}")
        for root, dirs, files in os.walk(directory_path + '/' + directory):
            for file in files:
                if file.endswith('.sol'):
                    contract_path = root + '/' + file
                    print(f"Path is: {contract_path}")
                    try:
                        with open(contract_path, "r", encoding='utf-8') as sol_file:
                            # print("UTF-8 Reading")
                        ...
                        with open(contract_path, 'rb') as sol_file:
                    ...
                    listener.write_csv(file, directory, result_path)
        i += 1
                    # Writing on CSV file
